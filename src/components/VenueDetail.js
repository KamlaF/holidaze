...
const VenueDetail = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto mt-8 px-4 sm:px-6 lg:px-8">
      <h2 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-6">{venue.name}</h2>
      <div className="mb-6">
        <img src={venue.media && venue.media.length > 0 ? venue.media[0] : ''} alt={venue.name} className="w-full h-auto rounded-lg shadow" />
      </div>
      ...
      {isAuthenticated && userRole !== 'manager' && (
        <>
          {bookingSuccess ? (
            <div className="bg-green-200 text-green-800 p-4 rounded mt-4 text-center">Booking successful! Redirecting...</div>
          ) : (
            <CreateBooking
              venueId={id}
