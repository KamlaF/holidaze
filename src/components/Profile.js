...
function Profile() {
  ...
  const onSubmit = async (data) => {
    try {
      const updatedUser = await updateAvatar({ avatarUrl: data.avatar, userName });
        ...
        {profile && (
          <>
            <div className="flex flex-col items-center mb-4">
              <img src={profile.avatar} alt="Profile Avatar" className="w-32 h-32 object-cover rounded-full"/>
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col space-y-2">
              ...
              />
              <button type="submit" className="bg-blue-500 text-white p-2 rounded hover:bg-blue-600 self-center">Update Avatar</button>
            </form>
          </>
        )}
      </div>
